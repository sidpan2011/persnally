...
| 2026-06-17 | **Market re-check + vision lock.** Adversarial deep-research (primary-sourced, 3-of-3 verified) found the two load-bearing positioning claims weakened-to-broken: the "open seat" is contested by shipping cross-vendor user-owned products (Plurality, Maximem, Mem0 OpenMemory), and incumbents shipped cross-app memory import (Anthropic + Google, Mar 2026) — so "incumbents can't/won't copy" is false in strong form. MCP distribution bet stands (97M→110M monthly SDK downloads, Linux Foundation governance). Surviving wedge = local-first plaintext + source-available + Claude Code/git ingestion. Locked the durable vision in [docs/VISION.md](./docs/VISION.md) (ownership-framed: "yours" is the promise, local-first is the proof — not "private"). Brand lines updated: descriptor "for you" → "your own context engine"; "they built it for agents, we for you" contrast retired (rivals say "for you" now). |
| 2026-06-18 | **Landing shipped + v1 backend removed** (two PRs off the vision lock). Landing (PR #40): descriptor → "your own context engine", Positioning reframed to ownership+custody, "nothing leaves your laptop" over-claim fixed, hero ownership beat, and the v1 cloud-synced web dashboard/auth UI removed (`web/src/lib/supabase` kept for Phase 4). Backend (this PR): full v1 Python/Supabase teardown — `api/`, `src/`, `supabase/` migrations, `requirements.txt`, Railway/nixpacks/ruff configs, the api CI workflow; stale `CONTRIBUTING.md`/docs synced; `experiments/` kept (Phase-0 capture-rate tooling). PIVOT.md competitive framing refreshed to mid-2026 reality. v2 is now local-first TS only. |
| 2026-06-20 | **Voice layer — Slice 2 (live dynamic capture)**. `persnally_track` gains an optional `style[]` so the connected AI emits voice/convention/emphasis signals *as the user chats* — zero extra inference, `basis: observed` — with anti-spam in the tool contract + a served cap (top 28 by confidence). `store.voice()` merges the stylometry backbone with live observed signals (dedupe by pattern, newest wins); dashboard voice section refreshes live (25s). 80 tests + e2e green (new: live capture writes `basis: observed`; `persnally_context` injects the voice pack); live-verified an observed signal merging into `/voice`. Gap flagged for Slice 3: style signals not yet deletable via UI (forget is topic-only). |
| 2026-06-20 | **Voice & convention layer — Slice 1** (context depth; [docs/CONTEXT_DEPTH.md](./docs/CONTEXT_DEPTH.md)). The prescriptive layer that makes connected tools answer *like the user*: new `signal.style` schema; deterministic local **stylometry** (`stylometry.ts`, zero-token — repeated phrases, terse/hedging/imperative/emoji/casing) emitted at import; **corpus hygiene** (`prose.ts`, `stripNoise`/`proseLines`) that also de-noises topic extraction + the LLM profile (a broader pipeline fix); `store.voice()` + `GET /voice`; `persnally_context` now injects "How to write for this user"; `persnallyd voice` (offline, re-runnable refresh — the dynamic hook); dashboard "How you write" section. 80 tests (5 new) + e2e green, live-verified on 3,271 real prompts. Slices 2–3 (live MCP style capture + `user.correction` loop) make it continuously dynamic. |
| 2026-06-20 | **Dashboard redesign** (`dashboard.html`). Rebuilt the localhost trust surface from a debug panel into a retention engine: hero self-portrait, "since you last looked" delta, `context.read` **receipts** ("what your AIs read about you"), reflections, and an interactive **interest constellation** (hand-rolled canvas force-graph — glow, category clustering, curved gradient edges, drag/zoom/hover; no CDN, offline). Monochrome dark (color only in the graph, green dot = active); collapsible cards, full-width title, GitHub/site CTAs; demo-data fallback previews without the daemon. Render-verified headlessly. |
| 2026-06-20 | **Market re-validation #2** (full adversarial deep-research, 23/25 claims verified — no new build). Verdict: the moat narrowed to **one verifiable axis** (local-plaintext + source-available "verify-it" custody) — rivals (Plurality, Maximem/Vity, Mem0) now also claim "user-owned"/"private"/"cross-vendor," so those words no longer differentiate. **New: the "Claude Code transcript ingestion is uncontested" wedge is FALSIFIED** — `claude-mem` (~65–83K stars) is a local-first SQLite+dashboard daemon in our exact form factor using the same SessionStart-injection pattern as `feat/connect-installs-hook`; the surviving distinction is *personal cross-AI model spanning chat exports* vs claude-mem's *project memory for the agent*. MCP rail re-confirmed (LF/AAIF, 10k+ servers, ~97M downloads). **Two open risks unverified, monitor before launch:** (1) incumbent two-way/programmatic memory (no evidence either way — NOT confirmed they stood still); (2) demand/retention (ProfileGPT-class portrait tools are wow-once-abandon; no third-party retention data for anyone). Conclusion: market can no longer validate — only the demand test can. Reframed `launch/COLD_DEMO.md` (objection handling for claude-mem/Plurality/incumbents; "portable model of you" + "verify-it" framing). No launch on the old "cross-vendor/user-owned" story. |
| 2026-06-18 | **Validate-track prep** (no new engine build). Dogfooded the live install → caught a stale-env mismatch on the dev machine (global `persnally@1.0.0` + daemon 2.0.0 vs published 2.0.3): updated the global to 2.0.3, restarted the launchd daemon onto the current build, reconnected all 3 MCP clients (claude-code/desktop/cursor) to v2; store (87 events) intact. Clean-room verified the fresh-user path (`npm i -g persnally` → 2.0.3, all bins, ~5s install). **Preliminary capture-rate signal: ~0% organic** — 25 claude-code sessions since 06-11 produced ~0 unprompted `persnally_track` calls (only smoke-test tool calls in telemetry) → reinforces *cold start by import, not accrual*; now instrumented across all 3 clients for the real ~June 25 verdict. Added the cold-demo runbook ([launch/COLD_DEMO.md](./launch/COLD_DEMO.md)), tuned to lead with the import/self-portrait wow. |
...
2. **Cold demos to 3–5 developers** — `npm i -g persnally && persnally setup` is the whole script; runbook at [launch/COLD_DEMO.md](./launch/COLD_DEMO.md). The real Phase 0/1 exit gate. **No new build work before this.** (Dev-machine env fixed to 2.0.3 on 2026-06-18, so a demo there runs v2.)
...
- **Superseded:** the Phase 0 "open seat: user-owned cross-vendor context layer" verdict (2026-06-11 scoreboard) was falsified by the 2026-06-17 market re-check, and narrowed further on 2026-06-20: positioning must lead with **verify-it custody** (local plaintext + source-available) and **"a portable model of *you*", not "memory"** — and must hold a distinction from `claude-mem` (personal cross-AI model vs project memory for the agent). "Cross-vendor"/"user-owned"/"private" are all contested words now (see [docs/VISION.md](./docs/VISION.md)). PIVOT.md competitive framing still needs a refresh to the 06-20 reality.
- **Open / monitor before launch (2026-06-20):** (1) have Anthropic/Google/OpenAI upgraded one-way cross-app memory *import* into two-way/programmatic/API memory? — unverified, load-bearing; (2) does anyone *retain* (not just install) users in personal AI memory? — no third-party data found, leaning vitamin; re-check both right before any public launch.
