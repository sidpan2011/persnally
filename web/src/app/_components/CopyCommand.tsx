...
export function CopyCommand({ command, className = "" }: { command: string; className?: string }) {
  ...
  return (
    <button
      ...
      className={`group flex items-center gap-3 rounded-xl border border-line bg-surface/80 px-4 py-3 text-left transition-colors hover:border-volt/50 ${className}`}
    >
      ...
      <span
        className={`shrink-0 font-mono text-[11px] uppercase tracking-wider transition-colors ${
          copied ? "text-volt" : "text-faint group-hover:text-mute"
        }`}
      >
        {copied ? "copied" : "copy"}
      </span>
