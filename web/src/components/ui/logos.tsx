/* Monochrome brand logos — official marks (simple-icons + OpenAI), rendered in
...
// OpenAI/ChatGPT was removed from simple-icons (trademark) — canonical mark inlined.
...
function Glyph({ icon, className = "size-4" }: { icon: SimpleIcon; className?: string }) {
  return (
    <svg role="img" viewBox="0 0 24 24" fill="currentColor" className={className} aria-label={icon.title}>
      <path d={icon.path} />
    </svg>
  );
}
...
export const GithubIcon = ({ className }: { className?: string }) => (
  <Glyph icon={siGithub} className={className} />
);
