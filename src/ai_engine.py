...
from .system_prompts import BEHAVIORAL_ANALYSIS_PROMPT, CONTENT_GENERATION_PROMPT, LOCATION_RULES
...
class AIEditorialEngine:
    ...
    async def generate_daily_5(
        self, user_profile: dict, research_data: dict, location_rule: str = None
    ) -> dict[str, Any]:
        ...
        for attempt in range(max_attempts):
            ...
            daily_5_content = await self._generate_content(
                user_profile, research_data, behavioral_data, location_rule, repo_files_data, web_opportunities
            )
            ...
            if validation_result["valid"]:
                ...
                if not url_verification["all_verified"]:
                    ...
                    if attempt < max_attempts - 1:
                        ...
                        working_items = [
                            item for i, item in enumerate(items) if i not in url_verification["failed_items"]
                        ]
                ...
                return {
                    ...
                    "date": current_date,
                    "summary": await self.opportunity_matcher.generate_opportunity_summary(
                        daily_5_content.get("items", []), behavioral_data
                    ),
                }
            else:
                ...
                if attempt < max_attempts - 1:
                    ...
                    if best_content and best_score > 50:
                        ...
                        current_date = datetime.now().strftime("%B %d, %Y")
                        subject_line = self.behavior_analyzer.get_intent_based_subject_line(
                            behavioral_data, current_date
                        )
                        personalization_note = self.behavior_analyzer.get_personalization_note(behavioral_data)
                        ...
                        return {
                            ...
                            "date": current_date,
                            "summary": await self.opportunity_matcher.generate_opportunity_summary(
                                best_content.get("items", []), behavioral_data
                            ),
                        }
                    else:
                        ...
                        raise Exception(
                            f"Content quality too low after {max_attempts} attempts (best score: {best_score}/100)"
                        )
    ...
    async def _analyze_behavior(self, research_data: dict, user_profile: dict) -> dict:
        ...
        prompt = BEHAVIORAL_ANALYSIS_PROMPT.format(
            ...
            languages=json.dumps(
                github_data.get("repo_analysis", {}).get("top_languages", [])[:5]
                if github_data.get("repo_analysis", {}).get("top_languages")
                else [],
                indent=2,
            ),
            recent_commits=json.dumps(
                github_data.get("repo_analysis", {}).get("recent_activity", [])[:5]
                if github_data.get("repo_analysis", {}).get("recent_activity")
                else [],
                indent=2,
            ),
            topics=json.dumps(
                github_data.get("repo_analysis", {}).get("topics", [])[:10]
                if github_data.get("repo_analysis", {}).get("topics")
                else [],
                indent=2,
            ),
    ...
    async def _generate_content(
        ...
        repo_files_data: dict = None,
        web_opportunities: dict = None,
    ) -> dict:
        ...
        try:
            ...
            repo_files_summary = (
                repo_files_data.get("summary", "No file details available")
                if repo_files_data
            ...
            if isinstance(opps, dict):
                ...
                # Combine for prompt
                opps_data = {"hackathons": hackathons, "jobs": jobs}
                ...
                print("⚠️ WARNING: opportunities is a list (unexpected), using as-is")
                opps_data = opps[:8]
            else:
        ...
        try:
            ...
            fallback_content = await self.content_curator.curate_geographically_relevant_content(
                user_profile, research_data, behavioral_data
            )
            ...
            if isinstance(fallback_content, list):
                ...
                if "items" in fallback_content:
                    print("✅ Fallback is dict with 'items' key")
