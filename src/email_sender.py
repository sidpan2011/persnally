...
class PremiumEmailSender:
    ...
    async def send_daily_5_newsletter(self, user_data: dict, daily_5_content: dict[str, Any]):
        ...
        success = await self.resend_client.send_email_via_mcp(user_data["email"], subject, html_content)
    ...
    def _generate_daily_5_email_html(self, user_data: dict, daily_5_content: dict[str, Any]) -> str:
        ...
        with open(TEMPLATE_DIR / "email.html") as f:
    ...
    def _markdown_to_html(self, markdown_text: str) -> str:
        ...
        html = re.sub(
            r"\[([^\]]+)\]\(([^\)]+)\)",
            r'<a href="\2" style="color: #2563eb; text-decoration: none; font-weight: normal;">\1</a>',
            html,
        )
    ...
    def _log_comprehensive_scores(self, user_data: dict, daily_5_content: dict[str, Any]):
        ...
        for i, item in enumerate(items, 1):
            ...
            composite_score = interest_match_score * 0.50 + content_quality * 0.30 + github_match_score * 0.20
            print(f"│      → Composite Score: {composite_score:.1f}/100")
            print("│         (50% user interests + 30% quality + 20% GitHub)")
